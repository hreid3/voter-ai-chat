...
export const vectorIndexTables = async (tableDds: VoterTableDdl[] = []): Promise<boolean> => {
    ...
    try {
        ...
        await client.unsafe(`CREATE INDEX IF NOT EXISTS idx_voter_table_embedding ON ${schemaName}.${voterTableName} USING ivfflat (table_embedding);`);
        await client.unsafe(`CREATE INDEX IF NOT EXISTS idx_voter_table_ddl_embeddings ON ${schemaName}.${chunkTableName} USING ivfflat (chunk_embedding);`);
